...
public class MergeSort : MonoBehaviour
{
    ...
    private void Merge(int[] arr, int left, int mid, int right)
    {
        ...
        int[] leftArr = new int[n1]; // 왼쪽 배열 크기 설정
        int[] rightArr = new int[n2]; // 오른쪽 배열 크기 설정

        for (int i = 0; i < n1; i++) // 왼쪽 배열 초기화
            leftArr[i] = arr[left + i];

        for (int i = 0; i < n2; i++) // 오른쪽 배열 초기화
